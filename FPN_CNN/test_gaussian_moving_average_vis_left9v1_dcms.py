    ...
    for f_idx in range(len(test_files)):
        ...
        for ii, data in tqdm(enumerate(data_loader)):
            ...
            with torch.no_grad():
                ...
                results_dcms = [x[0:1,:3].detach().cpu().numpy().astype(np.float64) for x in output["reg_dcms"]]
            for i, (scene_id, scene_primary_pedestrian_id, start_frame, end_frame, track_id, pred_traj, 
                    pred_traj_dcms, gt_past) in enumerate(zip(scene_ids, scene_primary_pedestrian_ids, 
                                          start_frames, end_frames, track_ids, results, results_dcms, gt_pasts)):
                scene_data = dict()
                ...
                preds_dcms = pred_traj_dcms.squeeze()
                preds_dcms = np.array([gaussian_smoothing(preds_dcms[s], sigma) 
                                       for s in range(preds_dcms.shape[0])])
                gt_past_cat = np.repeat(np.expand_dims(gt_past[57:58,:2], axis=0), repeats=3, axis=0)
                pred_gt_cat = np.concatenate([gt_past_cat, preds_dcms], axis=1)
                pred_gt_cat = np.array([moving_average(pred_gt_cat[s], window_size) 
                                  for s in range(pred_gt_cat.shape[0])])
