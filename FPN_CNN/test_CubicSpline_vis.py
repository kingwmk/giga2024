        ...
        for ii, data in tqdm(enumerate(data_loader)):
            ...
            for i, (scene_id, scene_primary_pedestrian_id, start_frame, end_frame, track_id,
                ...
                for j in range(3):  
                    ...
                    cs_x = CubicSpline(step_full, x_full)
                    cs_y = CubicSpline(step_full, y_full)

                    step_interp = np.linspace(60, 118, 59)
                    x_interp = cs_x(step_interp)
                    y_interp = cs_x(step_interp)
                    result[j,:-1, 0] = x_interp
                    result[j,:-1, 1] = y_interp
                    result[j,-1] = preds[j]           
                preds = result
