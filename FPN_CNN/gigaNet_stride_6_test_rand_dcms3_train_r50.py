...
config["train_split"] = root_path + 'train/preprocess_stride_6_test_dcms_r50/train.p'
config["val_split"] = root_path + 'train/preprocess_stride_6_test_dcms_r50/train_val.p'
...
class PredLoss(nn.Module):
    ...
    def forward(self, out: Dict[str, List[Tensor]], gt_preds: List[Tensor], 
        ...
        reg_min = reg[row_idcs, min_idcs]
        coef = self.config["reg_coef"]
        loss_out["reg_loss"] += coef * (self.reg_loss(reg_min[has_preds], gt_preds[has_preds]))
        loss_out["num_reg"] += has_preds.sum().item()
        
        coef = self.config["end_coef"]
        loss_out["end_loss"] += coef * (self.reg_loss(reg_min[row_idcs, last_idcs], gt_preds[row_idcs, last_idcs]))
        ...
        for i in range(len(reg)):
            ...
            for j in range(3):
                ...
                loss_out["con_loss"] += coef * (self.reg_loss(reg_tmp[reg_has_preds[i]], 
                                                      reg_dcms_tmp[has_preds_dcms[i]]))
        loss_out["num_con"] += 3*has_preds_dcms.sum().item()      
        return loss_out
