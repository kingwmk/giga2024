...
class Net(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        out["test_reg"] = test_ctrs
        out["test_cls"] = test_out["cls"]

        # transform prediction to world coordinates
        for i in range(len(out["reg"])):
            out["test_reg"][i] = torch.matmul(test_ctrs[i], rot[i]) + orig[i].view(1, 1, 1, -1)
...
class PredLoss(nn.Module):
    ...
    def forward(self, out: Dict[str, List[Tensor]], gt_preds: List[Tensor], 
        ...
        test_cls, test_reg = out["test_cls"], out["test_reg"]
        cls = torch.cat([x.unsqueeze(0) for x in cls], 0)
        reg = torch.cat([x for x in reg], 0).squeeze()
        test_cls = torch.cat([x.unsqueeze(0) for x in test_cls], 0)
        test_reg = torch.cat([x for x in test_reg], 0).squeeze()      
        ...
        loss_out["test_end_loss"] = zero.clone()
...
class PostProcess(nn.Module):
    ...
    def display(self, metrics, dt, epoch, lr=None):
        ...
        test_cls = metrics["test_cls_loss"] / (metrics["num_cls"] + 1e-10)
        test_end = metrics["test_end_loss"] / (metrics["num_end"] + 1e-10)
        loss = cls + end + test_cls + test_end
        ...
        print(
            "loss %2.4f cls %2.4f, end %2.4f, test_cls %2.4f, test_end %2.4f, ade1 %2.4f, ade %2.4f"
            % (loss, cls, end, test_cls, test_end, ade1, ade)
        )
        print()
