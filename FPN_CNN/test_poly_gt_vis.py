    ...
    for f_idx in range(len(test_files)):
        ...
        for ii, data in tqdm(enumerate(data_loader)):
            ...
            for i, (scene_id, scene_primary_pedestrian_id, start_frame, end_frame, track_id,
                ...
                preds = np.zeros_like(predicted_trajectories)
                for i in range(3):
                    for j in range(2):  # x 和 y 坐标分别处理
                        t_observed = np.arange(observed_L)
                        t_predicted = np.arange(observed_L, observed_L + predict_L)
                        t_combined = np.concatenate((t_observed, t_predicted))
                        coords_observed = observed_trajectories[i, :, j]
                        coords_predicted = predicted_trajectories[i, :, j]
                        coords_combined = np.concatenate((coords_observed, coords_predicted))

                        poly_coeffs = np.polyfit(t_combined, coords_combined, order)
                        poly = np.poly1d(poly_coeffs)

                        # 计算平滑轨迹
                        smoothed_combined = poly(t_combined)
                        preds[i, :, j] = smoothed_combined[observed_L:]
