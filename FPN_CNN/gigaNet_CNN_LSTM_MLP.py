...
class ActorNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.output_cnn = Res1d(n, n, norm=norm, ng=ng)
        self.fc_cnn = LinearRes(n, n, norm=norm, ng=ng)
      
        ctrs_in = 2
        self.lstm_h0_init_function = nn.Linear(ctrs_in, n, bias=False)
        self.lstm_encoder = nn.LSTM(3, n, num_layers=2, batch_first=True)
        self.dropout_lstm = nn.Dropout(0.1)
        self.fc_lstm = LinearRes(n, n, norm=norm, ng=ng)
      
        self.mlp_encoder = LinearRes(3*60, n, norm=norm, ng=ng)
        self.mlp_dropout = nn.Dropout(0.1)
        self.fc_mlp = LinearRes(n, n, norm=norm, ng=ng)

        self.output = LinearRes(n, n, norm=norm, ng=ng)
    ...
    def forward(self, actors: Tensor, actor_ctrs) -> Tensor:
        ...
        out_cnn = self.output_cnn(out)
        out_cnn = self.dropout_cnn(out_cnn[:, :, -1])
        out_cnn = self.fc_cnn(out_cnn)
       
        h0 = self.lstm_h0_init_function(actor_ctrs).view(1, M, config["n_actor"]).repeat(2, 1, 1)
        c0 = self.lstm_h0_init_function(actor_ctrs).view(1, M, config["n_actor"]).repeat(2, 1, 1)
        actors = actors.transpose(1, 2).contiguous()
        output, (hn, cn) = self.lstm_encoder(actors, (h0, c0))
        out_lstm = self.dropout_lstm(output[:, -1, :]).contiguous().view(M, config["n_actor"])
        out_lstm = self.fc_lstm(out_lstm)

        out_mlp = self.mlp_encoder(actors.view(M, L*d))
        out_mlp = self.dropout_mlp(out_mlp)
        out_mlp = self.fc_mlp(out_mlp)
      
        out = self.output(out_lstm + out_cnn + out_mlp)
