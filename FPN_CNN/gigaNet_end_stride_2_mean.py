...
config["train_split"] = root_path + 'train/preprocess_end_stride_2_mean/train_train.p'
config["val_split"] = root_path + 'train/preprocess_end_stride_2_mean/train_val.p'
...
class Net(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        actors, actor_idcs = actor_gather(gpu(data["feats"]), gpu(data["mean_feats"]))
        ...
        # prediction
        actor_means = gpu(data["means"])
        out = self.pred_net(actors, actor_idcs, actor_ctrs, actor_means)
...
def actor_gather(actors: List[Tensor], mean_actors) -> Tuple[Tensor, List[Tensor]]:
    batch_size = len(actors)
    num_actors = [len(x) for x in actors]

    actors = [x.transpose(1, 2) for x in actors]
    actors = torch.cat(actors, 0)
    mean_actors = [x.transpose(1, 2) for x in mean_actors]
    mean_actors = torch.cat(mean_actors, 0)
  
    actors = torch.cat((actors, mean_actors), 1)
...
class PredNet(nn.Module):
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor], actor_means
        ...
        agent_means = [actor_mean[0:1] for actor_mean in actor_means]
        ...
        for i in range(len(actor_idcs)):
            ...
            means = agent_means[i].view(-1, 1, 1, 2)
            reg[i] = reg[i] + ctrs + means
