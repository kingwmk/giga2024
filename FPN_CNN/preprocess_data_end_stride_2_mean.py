...
train_file = train_path + 'preprocess_end_stride_2_mean/train.p'
train_train_file = train_path + 'preprocess_end_stride_2_mean/train_train.p'
train_val_file = train_path + 'preprocess_end_stride_2_mean/train_val.p'
test_files = [test_path + 'preprocess_end/test_1.p', test_path + 'preprocess_end/test_2.p', 
              test_path + 'preprocess_end/test_3.p', test_path + 'preprocess_end/test_4.p', 
              test_path + 'preprocess_end/test_5.p', test_path + 'preprocess_end/test_6.p', 
              test_path + 'preprocess_end/test_7.p', test_path + 'preprocess_end/test_8.p', 
             ]
            ...
            for i in tqdm(range(num_scene)):
                ...
                for agent_current_step in range(1, agent_steps[-1], 2):
                    ...
                    for traj, step, pred_id in zip(trajs, steps, scene_pred_ids):
                        ...
                        mean_feat = np.zeros((60, 2), np.float32)
                        mean = np.average(feat[step[1:], :2], axis=0)
                        mean_feat[step[1:], :2] = feat[step[1:], :2] - mean
                    ...
                    mean_feats = np.asarray(mean_feats, np.float32)
    ...
    for f_idx,file in enumerate(test_set):
        ...
        with open(file) as f:
            ...
            for i in tqdm(range(num_scene)):
                ...
                scene_data['mean_feats'] = mean_feats
                scene_data['means'] = means 
