...
config["train_split"] = root_path + 'train/preprocess_end_stride_2_origin/train_train.p'
config["val_split"] = root_path + 'train/preprocess_end_stride_2_origin/train_val.p'
...
class Net(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        actors, actor_idcs = actor_gather(gpu(data["feats"]), gpu(data["origin_past_ctrs"]))
...
def actor_gather(actors: List[Tensor]) -> Tuple[Tensor, List[Tensor]]:
    ...
    origin_past_ctrs = [x.transpose(1, 2) for x in origin_past_ctrs]
    origin_past_ctrs = torch.cat(origin_past_ctrs, 0)
    
    actors = torch.cat((actors, origin_past_ctrs), 1)
