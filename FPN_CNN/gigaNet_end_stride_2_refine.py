...
class PredNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.att_dest_refine = AttDest(n_actor)
        self.refine = nn.Sequential(
            LinearRes(n_actor, n_actor, norm=norm, ng=ng), nn.Linear(n_actor, 2 * config["num_preds"])
        )
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor],
        ...
        reg_refine = self.refine(feats).view(reg.size(0), reg.size(1), -1, 2) + reg
                 
        dest_ctrs = reg_refine[:, :, -1].detach()
        feats = self.att_dest_refine(agents, torch.cat(agent_ctrs, 0), dest_ctrs)                 
        ...
        reg_refine = reg_refine[row_idcs, sort_idcs].view(cls.size(0), cls.size(1), -1, 2)
        out = dict()
        out["cls"], out["reg"], out["reg_refine"] = [], [], []
        for i in range(len(actor_idcs)):
            out["cls"].append(cls[i])
            out["reg"].append(reg[i])
            out["reg_refine"].append(reg_refine[i])
        return out
...
class PredLoss(nn.Module):
    ...
    def forward(self, out: Dict[str, List[Tensor]], gt_preds: List[Tensor], 
        ...
        cls, reg, reg_refine = out["cls"], out["reg"], out["reg_refine"]
        cls = torch.cat([x.unsqueeze(0) for x in cls], 0)
        reg = torch.cat([x for x in reg], 0).squeeze()
        reg_refine = torch.cat([x for x in reg_refine], 0).squeeze()
        ...
        loss_out["num_end"] = 0
        loss_out["refine_loss"] = zero.clone()
...
class PostProcess(nn.Module):
    ...
    def forward(self, out,data):
        post_out = dict()
        post_out["preds"] = [x.detach().cpu().numpy() for x in out["reg_refine"]]
    ...
    def display(self, metrics, dt, epoch, lr=None):
        ...
        refine = metrics["refine_loss"] / (metrics["num_end"] + 1e-10)
        loss = cls + end + refine
        ...
        print(
            "loss %2.4f cls %2.4f, end %2.4f, refine %2.4f, ade1 %2.4f, ade %2.4f"
            % (loss, cls, end, refine, ade1, ade)
        )
        print()
