...
train_file = train_path + 'preprocess_stride_6_dcms_obs32/train.p'
train_train_file = train_path + 'preprocess_stride_6_dcms_obs32/train_train.p'
train_val_file = train_path + 'preprocess_stride_6_dcms_obs32/train_val.p'
test_files = [test_path + 'preprocess_stride_6_dcms_obs32/test_1.p', 
              test_path + 'preprocess_stride_6_dcms_obs32/test_2.p', 
              test_path + 'preprocess_stride_6_dcms_obs32/test_3.p', 
              test_path + 'preprocess_stride_6_dcms_obs32/test_4.p', 
              test_path + 'preprocess_stride_6_dcms_obs32/test_5.p', 
              test_path + 'preprocess_stride_6_dcms_obs32/test_6.p', 
              test_path + 'preprocess_stride_6_dcms_obs32/test_7.p', 
              test_path + 'preprocess_stride_6_dcms_obs32/test_8.p', 
             ]
            ...
            for i in tqdm(range(num_scene)):
                ...
                for agent_current_step in range(start_index, agent_steps[-1], 6):
                    ...
                    for traj, step, pred_id in zip(trajs, steps, scene_pred_ids):
                        step_curr = step + (32 - agent_current_step - 1)
                        if 31 not in step_curr or 30 not in step_curr or 29 not in step_curr:
                            continue
                        valid_track_ids.append(pred_id)
                        gt_pred = np.zeros((60, 2), np.float32)
                        has_pred = np.zeros(60, bool)
                        future_mask = np.logical_and(step_curr >= 32, 
                                                     step_curr < 92)
                        future_step = step_curr[future_mask] - 32
                        ...
                        step_dcms = (step + (32 - (agent_current_step - 1) - 1))[:-1].copy()
                        traj = traj[:-1].copy()
                        gt_pred_dcms = np.zeros((60, 2), np.float32)
                        has_pred_dcms = np.zeros(60, bool)
                        future_mask = np.logical_and(step_dcms >= 32, 
                                                     step_dcms < 92)
                        future_step = step_dcms[future_mask] - 32
                        future_traj = traj[future_mask]
                        gt_pred_dcms[future_step] = future_traj[:,:2]
                        has_pred_dcms[future_step] = 1
            
                        obs_mask = np.logical_and(step_dcms >= 0, step_dcms < 32)
    ...
    for f_idx,file in enumerate(test_set):
        ...
        with open(file) as f:
            ...
            for i in tqdm(range(num_scene)):
                ...
                for traj, step, pred_id in zip(trajs, steps, scene_pred_ids):
                    step_curr = step - 28
                    if 31 not in step_curr or (30 not in step_curr):
                        continue
                    valid_track_ids.append(pred_id)
                    gt_pred = np.zeros((60, 2), np.float32)
                    has_pred = np.zeros(60, bool)
                    future_mask = np.logical_and(step_curr >= 32, step_curr < 92)
                    future_step = step_curr[future_mask] - 32
                    future_traj = traj[future_mask]
                    if vis_count < 10:
                        vis_future.append(future_traj)
